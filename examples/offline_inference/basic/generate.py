...
def main(args: dict):
    ...
    outputs = model.generate(prompts, sample_parameter)
...
if __name__ == "__main__":
    ...
    engine_group.set_defaults(
        model="LLM-Research/Llama-3.2-1B-Instruct", task="generate")
    sample_group = parser.add_argument_group("Sample Parameter")
    sample_group.add_argument("--max_tokens", type=int)
    sample_group.add_argument("--temperature", type=float)
    sample_group.add_argument("--top_p", type=float)
    sample_group.add_argument("--top_k", type=int)
    args: dict = vars(parser.parse_args())
    main(args=args)
